...
`HOTFIX_READY` requires a current exact `Candidate SHA` and durable `READY_FOR_CI evidence` for that same candidate. Any later PR-head change makes the hotfix record stale until development/review evidence and the managed record are refreshed.
...
`HOTFIX_BLOCKED` is fail-closed. Use it when no repository-approved emergency path exists, the blast radius cannot be bounded safely, required evidence cannot be produced, or neither a safe permanent correction nor a safe temporary mitigation can be justified.