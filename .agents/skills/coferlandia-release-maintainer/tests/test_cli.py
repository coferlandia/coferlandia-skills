...
SCRIPT = Path(__file__).resolve().parents[1] / "scripts" / "coferlandia-release-maintainer-cli.py"


def run_cli(repo: Path, *args: str) -> subprocess.CompletedProcess[str]:
    return subprocess.run([sys.executable, str(SCRIPT), "--repo", str(repo), *args], text=True, capture_output=True, check=False)
...
class ReleaseMaintainerCliTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        (self.repo / ".claude-plugin/plugin.json").write_text(json.dumps({"name": "demo", "version": "1.2.0"}) + "\n", encoding="utf-8")
        ...
        for script in ("validate_skill.py", "bump_version.py"):
            (self.repo / "_protocol/scripts" / script).write_text("raise SystemExit(0)\n", encoding="utf-8")
        (self.repo / ".agents/skills/private/secret.txt").write_text("private\n", encoding="utf-8")
    ...
    def test_check_rejects_changelog_version_drift(self) -> None:
        ...
        path.write_text(path.read_text(encoding="utf-8").replace("1.1.0", "1.0.0", 1), encoding="utf-8")
        result = run_cli(self.repo, "check", "--release-ready")
        self.assertEqual(result.returncode, 1)
        payload = json.loads(result.stdout)
        self.assertTrue(any("does not match metadata.version" in error for error in payload["errors"]))

    def test_release_ready_rejects_nonempty_unreleased(self) -> None:
        run_cli(self.repo, "render-readme", "--write")
        notes = self.repo / "RELEASE-NOTES.md"
        notes.write_text(notes.read_text(encoding="utf-8").replace("## Unreleased\n\n", "## Unreleased\n\n- pending\n\n"), encoding="utf-8")
        result = run_cli(self.repo, "check", "--release-ready")
        self.assertEqual(result.returncode, 1)
    ...
    def test_package_contains_release_notes_and_excludes_local_skill(self) -> None:
        ...
        result = run_cli(self.repo, "package", "--output", str(output), "--verify")
    ...
    def test_prepare_updates_versions_changelog_notes_and_readme(self) -> None:
        ...
        self.assertEqual(result.returncode, 0, result.stdout)
        self.assertEqual(json.loads((self.repo / ".claude-plugin/plugin.json").read_text())["version"], "1.3.0")
        self.assertIn('version: "1.2.0"', (self.repo / "skills/meta/sample-skill/SKILL.md").read_text())
        self.assertIn("## 1.2.0 — 2026-08-02", (self.repo / "skills/meta/sample-skill/CHANGELOG.md").read_text())
        self.assertIn("## v1.3.0 (2026-08-02)", (self.repo / "RELEASE-NOTES.md").read_text())
        self.assertIn("**v1.3.0 — 2026-08-02**", (self.repo / "README.md").read_text())
