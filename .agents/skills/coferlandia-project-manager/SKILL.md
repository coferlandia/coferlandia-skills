...
`repos_root` may point outside the PM home repository. The PM home repository is the
controller repo, not one of the managed project repositories.
...
1. Read or generate `.coferlandia/project-manager/config.json` from `templates/config.template.json`.
...
- `repos_root` may remain external to the PM home repository
